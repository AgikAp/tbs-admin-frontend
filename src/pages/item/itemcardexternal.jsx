  ...
  useEffect(() => {
    ...
    if (selected) {
      if (itemIndex === -1) {
        ...
        let special = tempItems[itemIndex].prices.findIndex(val => val.level === 'special')

        if (guest !== -1) {
          tempItems[itemIndex].prices[guest].price = item.price.special
        } else {
        ...
        if (reseller !== -1) {
          tempItems[itemIndex].prices[reseller].price = item.price.special
        } else {
          tempItems[itemIndex].prices.push(
            {
              level: 'reseller',
        ...
        if (special !== -1) {
          tempItems[itemIndex].prices[special].price = item.price.special
        } else {
          tempItems[itemIndex].prices.push(
            {
              level: 'special',
  ...
  return (
    <>
      <div className='relative'>
        <input type="checkbox" id={`item_eks_${index}`} className="checkbox absolute top-0 right-0 border-dark-5" checked={selected} onChange={() => setSelected(!selected)} name='checking_nickname' disabled={!editMode} />
        <label htmlFor={`item_eks_${index}`} className={`border-dashed border-[1px] border-dark-5 px-5 py-5 rounded-md block ${selected ? 'bg-slate-700' : 'bg-slate-800'}`}>
          <div className='font-semibold'>
            <span>{item.name}</span><span className='ml-2 font-light text-[14px]'>({item.status})</span>
          </div>
          <div className='pt-3 text-dark-6'>
            <span className='text-[14px]'>Pricing Detail</span>
            <div className='text-[12px] flex gap-5'>
              <span>{idrFormat(item.price.basic)} (Basic)</span>
              <span>{idrFormat(item.price.premium)} (Premium)</span>
              <span>{idrFormat(item.price.special)} (Special)</span>
            </div>
          </div>
        </label>
      </div>
